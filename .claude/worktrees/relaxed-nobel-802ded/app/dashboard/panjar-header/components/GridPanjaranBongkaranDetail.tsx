...
import {
  PanjarBongkaranDetail,
  filterPanjarBongkaranDetail
} from '@/lib/types/panjarheader.type';
import { useGetPanjarBongkaranDetail } from '@/lib/server/usePanjarheader';
...
interface Filter {
  ...
  limit: number;
  search: string;
  filters: typeof filterPanjarBongkaranDetail;
  sortBy: string;
  sortDirection: 'asc' | 'desc';
}

const GridPanjarBongkaranDetail = () => {
  ...
  const [rows, setRows] = useState<PanjarBongkaranDetail[]>([]);
  ...
  const [filters, setFilters] = useState<Filter>({
    ...
    limit: 30,
    search: '',
    filters: filterPanjarBongkaranDetail,
    sortBy: 'id',
    sortDirection: 'asc'
  });
  ...
  const {
    ...
    isLoading,
    refetch
  } = useGetPanjarBongkaranDetail(headerData?.id ?? 0, {
    ...filters,
    page: 1
  });
  ...
  const columns = useMemo((): Column<PanjarBongkaranDetail>[] => {
    return [
      {
        key: 'nomor',
        name: 'NO',
  ...
  async function handleKeyDown(
    args: CellKeyDownArgs<PanjarBongkaranDetail>,
  ...
  function handleCellClick(args: { row: PanjarBongkaranDetail }) {
  ...
  function getRowClass(row: PanjarBongkaranDetail) {
    const rowIndex = rows.findIndex((r) => r.id === row.id);
    return rowIndex === selectedRow ? 'selected-row' : '';
  }

  function rowKeyGetter(row: PanjarBongkaranDetail) {
    return row.id;
  }
        ...
        <div className="flex h-[38px] w-full flex-row items-center justify-between rounded-t-sm border-b border-border bg-background-grid-header px-2">
          ...
          <div className="flex flex-row items-center">
            <DraggableColumn
              ...
              userId={user.id}
              gridName="GridPanjarBongkaranDetail"
...
export default GridPanjarBongkaranDetail;
