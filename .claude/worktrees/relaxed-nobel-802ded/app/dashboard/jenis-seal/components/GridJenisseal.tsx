...
import FormJenisseal from './FormJenisseal';
import { useQueryClient } from 'react-query';
import {
  JenissealInput,
  jenissealSchema
} from '@/lib/validations/jenisseal.validation';
...
import { IJenisseal } from '@/lib/types/jenisseal.type';
import {
  clearOpenName,
  setClearLookup
...
const GridJenisseal = () => {
  ...
  const [pageDataCache, setPageDataCache] = useState<Map<number, IJenisseal[]>>(
    new Map()
  );

  const { mutateAsync: createJenisseal, isLoading: isLoadingCreate } =
    useCreateJenisseal();
  const { mutateAsync: updateJenisseal, isLoading: isLoadingUpdate } =
    useUpdateJenisseal();
  ...
  const { mutateAsync: deleteJenisseal, isLoading: isLoadingDelete } =
    useDeleteJenisseal();
  ...
  const [rows, setRows] = useState<IJenisseal[]>([]);
  ...
  const streamBufferRef = useRef<Map<number, IJenisseal[]>>(new Map());
  ...
  const forms = useForm<JenissealInput>({
    resolver: mode === 'delete' ? undefined : zodResolver(jenissealSchema),
    mode: 'onSubmit',
    defaultValues: {
      nama: '',
      keterangan: '',
  ...
  const { data: allJenisseal, isLoading: isLoadingJenisseal } = useGetJenisseal(
    {
      ...filters,
  ...
  const columns = useMemo((): Column<IJenisseal>[] => {
    return [
      {
        key: 'nomor',
        name: 'NO',
  ...
  function handleCellClick(args: { row: IJenisseal }) {
  ...
  const handleGoToLastPage = useCallback(async () => {
    ...
    try {
      const results = await Promise.all(
        pagesToFetch.map((p) =>
          getJenissealFn({ ...filters, page: p, limit: filters.limit })
        )
      );

      const newCache = new Map<number, IJenisseal[]>();
  ...
  const onSuccess = async (
    indexOnPage: number,
    fetchedPages: number[],
    pagedData: Record<string, IJenisseal[]>,
    pageNumber: number,
    keepOpenModal = false,
    ...
    try {
      ...
      if (mode !== 'delete') {
        const response = await api2.get(
          `/redis/get/jenisseal-page-${pageNumber}`
        );
        setRows([]);
        ...
        Object.entries(pagedData).forEach(([key, value]) => {
          updatedBuffer.set(Number(key), value as IJenisseal[]);
        });
  ...
  const onSubmit = async (values: JenissealInput, keepOpenModal = false) => {
    ...
    try {
      ...
      if (mode === 'delete') {
        if (selectedRowId) {
          await deleteJenisseal(selectedRowId as unknown as string, {
            onSuccess: () => {
              setPopOver(false);
      ...
      if (mode === 'add') {
        const newOrder = await createJenisseal(
          {
            ...values,
            ...filters
          },
      ...
      if (selectedRowId && mode === 'edit') {
        ...
        queryClient.invalidateQueries('jenisseals');
      }
    } catch (error) {
      console.error(error);
    } finally {
  ...
  const handleReport = async () => {
    try {
      ...
      const response = await getJenissealFn(filtersWithoutLimit);
      const reportRows = response.data.map((row) => ({
        ...row,
        judullaporan: 'Laporan Jenis Seal',
      ...
      import('stimulsoft-reports-js/Scripts/stimulsoft.blockly.editor')
        .then((module) => {
          ...
          report.loadFile('/reports/LaporanJenisseal.mrt');
          ...
          report.renderAsync(() => {
            report.exportDocumentAsync((pdfData: any) => {
              ...
              window.open('/reports/jenisseal', '_blank');
  ...
  function getRowClass(row: IJenisseal) {
    const rowIndex = rows.findIndex((r) => r.id === row.id);
    return rowIndex === selectedRow ? 'selected-row' : '';
  }

  function rowKeyGetter(row: IJenisseal) {
    return row.id;
  }
  ...
  const handleAdd = async () => {
    try {
      ...
      console.error('Error add jenis seal:', error);
    }
  };
  ...
  const prefetchPages = useCallback(
    async (
      pagesToFetch: number[],
      existingCache?: Map<number, IJenisseal[]>,
      ...
      await Promise.allSettled(
        validPages.map(async (pageNum) => {
          try {
            const data = await getJenissealFn({
              ...filters,
              page: pageNum,
              limit: filters.limit
            });
  ...
  useEffect(() => {
    const handleBulkFetch = async () => {
      ...
      const bulkData = allJenisseal.data || [];
      ...
      const totalItems = allJenisseal.pagination?.totalItems || 0;
  ...
  useEffect(() => {
    ...
    if (allJenisseal.pagination?.totalPages) {
      setTotalPages(allJenisseal.pagination.totalPages);
    }
      ...
      >
        ...
        <div className="flex flex-row justify-between border border-x-0 border-b-0 border-border bg-background-grid-header p-2">
          <ActionButton
            ...
            totalItems={allJenisseal ? allJenisseal.pagination.totalItems : 0}
            startRow={startRow}
            customActions={[
              {
                label: 'Print',
          ...
          />
          {isLoadingJenisseal ? <LoadRowsRenderer /> : null}
          {contextMenu && (
            <div
              ref={contextMenuRef}
