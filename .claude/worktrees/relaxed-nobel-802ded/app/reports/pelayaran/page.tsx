...
import { exportPelayaranFn } from '@/lib/apis/pelayaran.api';
...
const ReportPelayaranPage: React.FC = () => {
  ...
  const handleExport = async () => {
    try {
      ...
      const response = await exportPelayaranFn(exportPayload);

      // Buat link download dari Blob
      const url = window.URL.createObjectURL(new Blob([response]));
      const link = document.createElement('a');
      link.href = url;
      link.download = `laporan_pelayaran_${Date.now()}.xlsx`;
      ...
      console.error('Error exporting pelayaran data:', error);
    }
  };
...
export default ReportPelayaranPage;
...
//       try {
//         const res = await getPelayaranFn({ page: 1, limit: 50 }); // sesuaikan filter
