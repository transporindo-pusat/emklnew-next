...
import { exportJabatanFn } from '@/lib/apis/jabatan.api';
...
const ReportMenuPage: React.FC = () => {
  ...
  const handleExport = async () => {
    try {
      ...
      const response = await exportJabatanFn(exportPayload);

      // Buat link download dari Blob
      const url = window.URL.createObjectURL(new Blob([response]));
      const link = document.createElement('a');
      link.href = url;
      link.download = `laporan_jabatan_${Date.now()}.xlsx`;
      ...
      console.error('Error exporting jabatan data:', error);
    }
  };
