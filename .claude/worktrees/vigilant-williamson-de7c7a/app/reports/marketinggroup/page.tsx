...
import { exportMarketingGroupFn } from '@/lib/apis/marketinggroup.api';
...
const ReportPelayaranPage: React.FC = () => {
  ...
  const handleExport = async () => {
    try {
      ...
      const response = await exportMarketingGroupFn(exportPayload);

      // Buat link download dari Blob
      const url = window.URL.createObjectURL(new Blob([response]));
      const link = document.createElement('a');
      link.href = url;
      link.download = `laporan_marketinggroup_${Date.now()}.xlsx`;
      ...
      console.error('Error exporting marketing group data:', error);
    }
  };
