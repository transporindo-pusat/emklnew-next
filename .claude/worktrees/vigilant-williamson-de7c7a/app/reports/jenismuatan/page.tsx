...
import { exportJenisMuatanFn } from '@/lib/apis/jenismuatan.api';
...
const ReportMenuPage: React.FC = () => {
  ...
  const handleExport = async () => {
    try {
      ...
      const response = await exportJenisMuatanFn(exportPayload);

      const url = window.URL.createObjectURL(new Blob([response]));
      const link = document.createElement('a');
      link.href = url;
      link.download = `laporan_jenismuatan_${Date.now()}.xlsx`;
      ...
      console.error('Error exporting jenismuatan data:', error);
    }
  };
