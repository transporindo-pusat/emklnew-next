...
import { exportSandarKapalFn } from '@/lib/apis/sandarkapal.api';
...
const ReportMenuPage: React.FC = () => {
  ...
  const handleExport = async () => {
    try {
      ...
      const response = await exportSandarKapalFn(exportPayload);

      // Buat link download dari Blob
      const url = window.URL.createObjectURL(new Blob([response]));
      const link = document.createElement('a');
      link.href = url;
      link.download = `laporan_sandarkapal_${Date.now()}.xlsx`;
      ...
      console.error('Error exporting sandarkapal data:', error);
    }
  };
  const onPrint = () => {
