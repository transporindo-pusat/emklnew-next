...
import { exportGandenganFn } from '@/lib/apis/gandengan.api';
...
const ReportMenuPage: React.FC = () => {
  ...
  const handleExport = async () => {
    try {
      ...
      const response = await exportGandenganFn(exportPayload);

      const url = window.URL.createObjectURL(new Blob([response]));
      const link = document.createElement('a');
      link.href = url;
      link.download = `laporan_gandengan_${Date.now()}.xlsx`;
      ...
      console.error('Error exporting gandengan data:', error);
    }
  };
