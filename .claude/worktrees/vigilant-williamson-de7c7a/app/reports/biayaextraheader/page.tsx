...
import { exportBiayaExtraHeaderFn } from '@/lib/apis/biayaextraheader.api';
...
const ReportMenuPage: React.FC = () => {
  ...
  const handleExport = async () => {
    try {
      const response = await exportBiayaExtraHeaderFn(Number(savedId));

      // Buat link download dari Blob
      const url = window.URL.createObjectURL(new Blob([response]));
      const link = document.createElement('a');
      link.href = url;
      link.download = `laporan_biaya_extra_${Date.now()}.xlsx`;
