...
const GridContainer = () => {
  ...
  const [pageDataCache, setPageDataCache] = useState<Map<number, IContainer[]>>(
    new Map()
  );
  ...
  const streamBufferRef = useRef<Map<number, IContainer[]>>(new Map());
  ...
  const forms = useForm<ContainerInput>({
    resolver: mode === 'delete' ? undefined : zodResolver(containerSchema),
    mode: 'onSubmit',
    defaultValues: {
      nama: '',
      keterangan: '',
  ...
  const handleGoToLastPage = useCallback(async () => {
    ...
    try {
      const results = await Promise.all(
        pagesToFetch.map((p) =>
          getContainerFn({ ...filters, page: p, limit: filters.limit })
        )
      );

      const newCache = new Map<number, IContainer[]>();
  ...
  const onSuccess = async (
    indexOnPage: number,
    fetchedPages: number[],
    pagedData: Record<string, IContainer[]>,
    pageNumber: number,
    keepOpenModal = false,
    ...
    try {
      ...
      if (mode !== 'delete') {
        const response = await api2.get(
          `/redis/get/container-page-${pageNumber}`
        );
        setRows([]);
        ...
        Object.entries(pagedData).forEach(([key, value]) => {
          updatedBuffer.set(Number(key), value as IContainer[]);
        });
    ...
    try {
      ...
      import('stimulsoft-reports-js/Scripts/stimulsoft.blockly.editor')
        .then((module) => {
          ...
          report.loadFile('/reports/LaporanJeniscontainer.mrt');
  ...
  const handleAdd = async () => {
    try {
      ...
      console.error('Error add container:', error);
    }
  };
  ...
  const prefetchPages = useCallback(
    async (
      pagesToFetch: number[],
      existingCache?: Map<number, IContainer[]>,
      ...
      await Promise.allSettled(
        validPages.map(async (pageNum) => {
          try {
            const data = await getContainerFn({
              ...filters,
              page: pageNum,
              limit: filters.limit
            });
  ...
  useEffect(() => {
    const handleBulkFetch = async () => {
      ...
      const bulkData = allContainer.data || [];
      ...
      const totalItems = allContainer.pagination?.totalItems || 0;
      ...
      >
        ...
        <div className="flex flex-row justify-between border border-x-0 border-b-0 border-border bg-background-grid-header p-2">
          <ActionButton
            ...
            totalItems={allContainer ? allContainer.pagination.totalItems : 0}
            startRow={startRow}
            customActions={[
              {
                label: 'Print',
