...
import { exportBiayaemklFn } from '@/lib/apis/biayaemkl.api';
...
const ReportMenuPage: React.FC = () => {
  ...
  const handleExport = async () => {
    try {
      ...
      const response = await exportBiayaemklFn(exportPayload);

      const url = window.URL.createObjectURL(new Blob([response]));
      const link = document.createElement('a');
      link.href = url;
      link.download = `laporan_biayaemkl_${Date.now()}.xlsx`;
      ...
      console.error('Error exporting biayaemkl data:', error);
    }
  };
