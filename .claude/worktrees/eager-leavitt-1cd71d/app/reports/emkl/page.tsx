...
import { exportEmklFn } from '@/lib/apis/emkl.api';
...
const ReportEmklPage: React.FC = () => {
  ...
  const handleExport = async () => {
    try {
      ...
      const response = await exportEmklFn(exportPayload);

      // Buat link download dari Blob
      const url = window.URL.createObjectURL(new Blob([response]));
      const link = document.createElement('a');
      link.href = url;
      link.download = `laporan_emkl_${Date.now()}.xlsx`;
      ...
      console.error('Error exporting emkl data:', error);
    }
  };
...
//       try {
//         const res = await getEmklFn({ page: 1, limit: 50 }); // sesuaikan filter
