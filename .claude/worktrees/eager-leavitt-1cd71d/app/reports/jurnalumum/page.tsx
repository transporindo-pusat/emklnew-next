...
import { exportJurnalUmumFn } from '@/lib/apis/jurnalumumheader.api';
...
const ReportMenuPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (storedId) {
      ...
      setSavedId(Number.isFinite(parsedId) ? parsedId : null);
    }
  }, []);
  ...
  const handleExport = async (): Promise<void> => {
    try {
      ...
      const response = await exportJurnalUmumFn(savedId, exportPayload);

      const url = window.URL.createObjectURL(new Blob([response]));
      const link = document.createElement('a');
      link.href = url;
      link.download = `laporan_jurnalumum_${Date.now()}.xlsx`;
      document.body.appendChild(link);
      link.click();
      window.URL.revokeObjectURL(url);
      document.body.removeChild(link);
    } catch (error) {
      console.error('Error exporting jurnal umum data:', error);
    }
  };
