...
import { exportTradoFn } from '@/lib/apis/trado.api';
...
const ReportMenuPage: React.FC = () => {
  ...
  const handleExport = async () => {
    try {
      ...
      const response = await exportTradoFn(exportPayload);

      const url = window.URL.createObjectURL(new Blob([response]));
      const link = document.createElement('a');
      link.href = url;
      link.download = `laporan_trado_${Date.now()}.xlsx`;
      ...
      console.error('Error exporting trado data:', error);
    }
  };
