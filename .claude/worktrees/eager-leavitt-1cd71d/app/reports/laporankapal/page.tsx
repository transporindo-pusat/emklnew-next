...
import { exportKapalFn } from '@/lib/apis/kapal.api';
...
const ReportMenuPage: React.FC = () => {
  ...
  const handleExport = async () => {
    try {
      ...
      const response = await exportKapalFn(exportPayload);

      // Buat link download dari Blob
      const url = window.URL.createObjectURL(new Blob([response]));
      const link = document.createElement('a');
      link.href = url;
      link.download = `laporan_kapal_${Date.now()}.xlsx`;
      ...
      console.error('Error exporting kapal data:', error);
    }
  };
...
//       try {
//         const res = await getKapalFn({ page: 1, limit: 50 }); // sesuaikan filter
