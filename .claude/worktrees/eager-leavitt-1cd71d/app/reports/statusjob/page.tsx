...
import { exportStatusJobMasukGudangFn } from '@/lib/apis/statusjob.api';
...
const ReportMenuPage: React.FC = () => {
  ...
  const [savedTglStatus, setSavedTglStatus] = useState<string>('');
  ...
  useEffect(() => {
    ...
    const storedTglStatus = sessionStorage.getItem('tglstatus');
    if (storedTglStatus) setSavedTglStatus(storedTglStatus);
  }, []);

  const handleExport = async (): Promise<void> => {
    try {
      if (!savedTglStatus) return;
      const exportPayload = { ...savedFilters };
      const response = await exportStatusJobMasukGudangFn(
        savedTglStatus,
        exportPayload
      );
      ...
      link.href = url;
      link.download = `laporan_status_job_${Date.now()}.xlsx`;
      document.body.appendChild(link);
      link.click();
      window.URL.revokeObjectURL(url);
      document.body.removeChild(link);
    } catch (error) {
      console.error('Error exporting status job data:', error);
    }
  };
