...
import { exportPenerimaanEmklFn } from '@/lib/apis/penerimaanemkl.api';
...
const ReportMenuPage: React.FC = () => {
  ...
  const handleExport = async () => {
    try {
      ...
      const response = await exportPenerimaanEmklFn(exportPayload);

      const url = window.URL.createObjectURL(new Blob([response]));
      const link = document.createElement('a');
      link.href = url;
      link.download = `laporan_penerimaanemkl_${Date.now()}.xlsx`;
      ...
      console.error('Error exporting penerimaanemkl data:', error);
    }
  };
  const onPrint = () => {
