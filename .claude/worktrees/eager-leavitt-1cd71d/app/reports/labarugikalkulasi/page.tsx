...
import { exportLabaRugiKalkulasiFn } from '@/lib/apis/labarugikalkulasi.api';
...
const ReportMenuPage: React.FC = () => {
  ...
  const handleExport = async () => {
    try {
      ...
      const response = await exportLabaRugiKalkulasiFn(exportPayload);

      const url = window.URL.createObjectURL(new Blob([response]));
      const link = document.createElement('a');
      link.href = url;
      link.download = `laporan_labarugikalkulasi_${Date.now()}.xlsx`;
      ...
      console.error('Error exporting labarugikalkulasi data:', error);
    }
  };
  const onPrint = () => {
