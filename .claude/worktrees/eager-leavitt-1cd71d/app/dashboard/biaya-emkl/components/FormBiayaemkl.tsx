...
const FormBiayaemkl = ({
  ...
  // LookUp tidak dibungkus FormField, jadi FormMessage tidak bisa dipakai. Tanpa ini
  // kegagalan validasi zod maupun penolakan 400 dari backend tidak terlihat sama
  // sekali — tombol SAVE seolah tidak melakukan apa-apa.
  const { errors: formErrors } = useFormState({ control: forms.control });
  const { errors: apiErrors } = useFormError();

  const lookupError = (name: string): string | undefined => {
    const zodMessage = (formErrors as Record<string, any>)?.[name]?.message;
    return (zodMessage as string | undefined) ?? apiErrors?.[name];
  };

  const LookupError = ({ name }: { name: string }) => {
    const message = lookupError(name);
    if (!message) return null;
    return (
      <p className="pt-1 text-[0.8rem] font-medium text-destructive">
        {message}
      </p>
    );
  };
                ...
                <div className="flex h-[100%] flex-col gap-2 lg:gap-3">
                  ...
                  <div className="flex w-full flex-col justify-between lg:flex-row lg:items-center">
                    ...
                    <div className="w-full lg:w-[85%]">
                      ...
                      <LookupError name="jenisorder_id" />
                    </div>
                  </div>
                  ...
                  <div className="flex w-full flex-col justify-between lg:flex-row lg:items-center">
                    ...
                    <div className="w-full lg:w-[85%]">
                      ...
                      <LookupError name="statusbiayabl" />
                    </div>
                  </div>
