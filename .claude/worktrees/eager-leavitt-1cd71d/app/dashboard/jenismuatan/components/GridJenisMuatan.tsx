...
const GridJenisMuatan = () => {
  ...
  const streamBufferRef = useRef<Map<number, IJenisMuatan[]>>(new Map());
  ...
  const forms = useForm<JenisMuatanInput>({
    resolver: mode === 'delete' ? undefined : zodResolver(jenismuatanSchema),
    mode: 'onSubmit',
    defaultValues: {
      nama: '',
      keterangan: '',
  ...
  const handleGoToLastPage = useCallback(async () => {
    ...
    try {
      const results = await Promise.all(
        pagesToFetch.map((p) =>
          getJenisMuatanFn({ ...filters, page: p, limit: filters.limit })
        )
      );

      const newCache = new Map<number, IJenisMuatan[]>();
  ...
  const onSuccess = async (
    indexOnPage: number,
    fetchedPages: number[],
    pagedData: Record<string, IJenisMuatan[]>,
    pageNumber: number,
    keepOpenModal = false,
    ...
    try {
      ...
      if (mode !== 'delete') {
        const response = await api2.get(
          `/redis/get/jenismuatan-page-${pageNumber}`
        );
        setRows([]);
        ...
        Object.entries(pagedData).forEach(([key, value]) => {
          updatedBuffer.set(Number(key), value as IJenisMuatan[]);
        });
  ...
  const handleAdd = async () => {
    try {
      ...
      console.error('Error add jenis muatan:', error);
    }
  };
  ...
  const prefetchPages = useCallback(
    async (
      pagesToFetch: number[],
      existingCache?: Map<number, IJenisMuatan[]>,
      ...
      await Promise.allSettled(
        validPages.map(async (pageNum) => {
          try {
            const data = await getJenisMuatanFn({
              ...filters,
              page: pageNum,
              limit: filters.limit
            });
  ...
  useEffect(() => {
    const handleBulkFetch = async () => {
      ...
      const bulkData = allJenisMuatan.data || [];
      ...
      const totalItems = allJenisMuatan.pagination?.totalItems || 0;
  ...
  useEffect(() => {
    const combinedRows: IJenisMuatan[] = [];
      ...
      >
        ...
        <div className="flex flex-row justify-between border border-x-0 border-b-0 border-border bg-background-grid-header p-2">
          <ActionButton
            ...
            totalItems={
              allJenisMuatan ? allJenisMuatan.pagination.totalItems : 0
            }
            startRow={startRow}
            customActions={[
              {
