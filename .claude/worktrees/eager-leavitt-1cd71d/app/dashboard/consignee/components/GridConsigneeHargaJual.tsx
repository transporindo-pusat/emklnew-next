...
const GridConsigneeHargaJual = ({ activeTab }: { activeTab: string }) => {
  ...
  const {
    ...
    activeTab === 'consigneehargajual' ? filters : {}
  );

  const [rows, setRows] = useState<IConsigneeHargaJual[]>([]);
  ...
  const columns = useMemo((): Column<IConsigneeHargaJual>[] => {
    return [
      {
        key: 'nomor',
        name: 'NO',
      ...
      },
      {
        ...
        renderHeaderCell: (column: any) => (
          <div className="flex h-full cursor-pointer flex-col items-center gap-1">
            <div
              ...
              onClick={() => handleSort('container_nama')}
              onContextMenu={(event) =>
            ...
            <div className="relative h-[50%] w-full px-1">
              <FilterInput
                colKey="container_nama"
                value={filters.filters.container_nama || ''}
                onChange={(value) =>
                  handleFilterInputChange('container_nama', value)
                }
                onClear={() => handleClearFilter('container_nama')}
                inputRef={(el) => {
                  inputColRefs.current['container_nama'] = el;
                }}
              />
            </div>
          </div>
        ...
        renderCell: (props: any) => {
          const columnFilter = filters.filters.container_nama || '';
  ...
  function getRowClass(row: IConsigneeHargaJual) {
    const rowIndex = rows.findIndex((r) => r.id === row.id);
    return rowIndex === selectedRow ? 'selected-row' : '';
  }
  function handleCellClick(args: { row: IConsigneeHargaJual }) {
  ...
  async function handleKeyDown(
    args: CellKeyDownArgs<IConsigneeHargaJual>,
  ...
  useEffect(() => {
    if (headerData?.id) {
      setFilters((prev) => ({
        ...prev,
        filters: {
          ...filterConsigneeHargaJual, // <--- semua filter dikosongkan dulu
        ...
        <div className="flex h-[38px] w-full flex-row items-center justify-between rounded-t-sm border-b border-border bg-background-grid-header px-2">
          ...
          <div className="flex flex-row items-center">
            <DraggableColumn
              ...
              userId={user.id}
              gridName="GridConsigneeHargaJual"
...
export default GridConsigneeHargaJual;
