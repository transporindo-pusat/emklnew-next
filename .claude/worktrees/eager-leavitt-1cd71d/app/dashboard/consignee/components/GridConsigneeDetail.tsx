...
import { useGetConsigneeDetail } from '@/lib/server/useConsignee';
import { IConsigneeDetail } from '@/lib/types/consignee.type';
...
const GridConsigneeDetail = ({ activeTab }: { activeTab: string }) => {
  ...
  const {
    ...
    isLoading,
    refetch
  } = useGetConsigneeDetail(activeTab === 'consigneedetail' ? filters : {});

  const [rows, setRows] = useState<IConsigneeDetail[]>([]);
  ...
  const columns = useMemo((): Column<IConsigneeDetail>[] => {
    return [
      {
        key: 'nomor',
        name: 'NO',
  ...
  function getRowClass(row: IConsigneeDetail) {
    const rowIndex = rows.findIndex((r) => r.id === row.id);
    return rowIndex === selectedRow ? 'selected-row' : '';
  }
  function handleCellClick(args: { row: IConsigneeDetail }) {
  ...
  async function handleKeyDown(
    args: CellKeyDownArgs<IConsigneeDetail>,
...
export default GridConsigneeDetail;
