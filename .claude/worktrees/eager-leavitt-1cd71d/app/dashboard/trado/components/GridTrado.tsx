...
import FormTrado from './FormTrado';
import { useQueryClient } from 'react-query';
import { TradoInput, tradoSchema } from '@/lib/validations/trado.validation';
...
import { ITrado } from '@/lib/types/trado.type';
import {
  clearOpenName,
  setClearLookup
...
import { getTradoFn } from '@/lib/apis/trado.api';
...
const GridTrado = () => {
  ...
  const [pageDataCache, setPageDataCache] = useState<Map<number, ITrado[]>>(
    new Map()
  );

  const { mutateAsync: createTrado, isLoading: isLoadingCreate } =
    useCreateTrado();
  const { mutateAsync: updateTrado, isLoading: isLoadingUpdate } =
    useUpdateTrado();
  ...
  const { mutateAsync: deleteTrado, isLoading: isLoadingDelete } =
    useDeleteTrado();
  ...
  const [rows, setRows] = useState<ITrado[]>([]);
  ...
  const streamBufferRef = useRef<Map<number, ITrado[]>>(new Map());
  ...
  const forms = useForm<TradoInput>({
    resolver: mode === 'delete' ? undefined : zodResolver(tradoSchema),
    mode: 'onSubmit',
    defaultValues: {
      nama: '',
      keterangan: '',
  ...
  const { data: allTrado, isLoading: isLoadingTrado } = useGetTrado(
    {
      ...filters,
  ...
  const columns = useMemo((): Column<ITrado>[] => {
    return [
      {
        key: 'nomor',
        name: 'NO',
  ...
  const onColumnsReorder = (sourceKey: string, targetKey: string) => {
    setColumnsOrder((prevOrder) => {
      ...
      saveGridConfig(String(user.id), 'GridTrado', [...newOrder], columnsWidth);
      return newOrder;
    });
  };

  async function handleScroll(event: React.UIEvent<HTMLDivElement>) {
    if (isLoadingTrado || rows.length === 0 || isTransitioning || isFetching)
      return;
  ...
  function handleCellClick(args: { row: ITrado }) {
  ...
  const handleGoToLastPage = useCallback(async () => {
    ...
    try {
      const results = await Promise.all(
        pagesToFetch.map((p) =>
          getTradoFn({ ...filters, page: p, limit: filters.limit })
        )
      );

      const newCache = new Map<number, ITrado[]>();
  ...
  const onSuccess = async (
    indexOnPage: number,
    fetchedPages: number[],
    pagedData: Record<string, ITrado[]>,
    pageNumber: number,
    keepOpenModal = false,
    ...
    try {
      ...
      if (mode !== 'delete') {
        const response = await api2.get(`/redis/get/trado-page-${pageNumber}`);
        setRows([]);
        ...
        Object.entries(pagedData).forEach(([key, value]) => {
          updatedBuffer.set(Number(key), value as ITrado[]);
        });
  ...
  const onSubmit = async (values: TradoInput, keepOpenModal = false) => {
    clearError();
    ...
    try {
      ...
      if (mode === 'delete') {
        if (selectedRowId) {
          await deleteTrado(selectedRowId as unknown as string, {
            onSuccess: () => {
              setPopOver(false);
      ...
      if (mode === 'add') {
        const newOrder = await createTrado(
          {
            ...values,
            ...filters
          },
      ...
      if (selectedRowId && mode === 'edit') {
        ...
        queryClient.invalidateQueries('trados');
      }
    } catch (error: any) {
  ...
  const handleReport = async () => {
    try {
      ...
      const response = await getTradoFn(filtersWithoutLimit);
      const reportRows = response.data.map((row) => ({
        ...row,
        judullaporan: 'Laporan Trado',
      ...
      import('stimulsoft-reports-js/Scripts/stimulsoft.blockly.editor')
        .then((module) => {
          ...
          report.loadFile('/reports/LaporanTrado.mrt');
          ...
          report.renderAsync(() => {
            report.exportDocumentAsync((pdfData: any) => {
              ...
              window.open('/reports/trado', '_blank');
  ...
  function getRowClass(row: ITrado) {
    const rowIndex = rows.findIndex((r) => r.id === row.id);
    return rowIndex === selectedRow ? 'selected-row' : '';
  }

  function rowKeyGetter(row: ITrado) {
    return row.id;
  }
  ...
  const handleAdd = async () => {
    try {
      ...
      console.error('Error add trado:', error);
    }
  };
  ...
  const prefetchPages = useCallback(
    async (
      pagesToFetch: number[],
      existingCache?: Map<number, ITrado[]>,
  ...
  useEffect(() => {
    const handleBulkFetch = async () => {
      if (!shouldBulkFetch || !allTrado || isDataUpdated || isAfterMutation) {
        return;
      }

      const bulkData = allTrado.data || [];
      ...
      const totalItems = allTrado.pagination?.totalItems || 0;
      ...
      >
        ...
        <div className="flex flex-row justify-between border border-x-0 border-b-0 border-border bg-background-grid-header p-2">
          <ActionButton
            ...
            totalItems={allTrado ? allTrado.pagination.totalItems : 0}
            startRow={startRow}
            customActions={[
              {
                label: 'Print',
