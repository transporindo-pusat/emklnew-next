...
import FormGandengan from './FormGandengan';
import { useQueryClient } from 'react-query';
import {
  GandenganInput,
  gandenganSchema
} from '@/lib/validations/gandengan.validation';
...
import { IGandengan } from '@/lib/types/gandengan.type';
import {
  clearOpenName,
  setClearLookup
...
import { getGandenganFn } from '@/lib/apis/gandengan.api';
...
const GridGandengan = () => {
  ...
  const [pageDataCache, setPageDataCache] = useState<Map<number, IGandengan[]>>(
    new Map()
  );

  const { mutateAsync: createGandengan, isLoading: isLoadingCreate } =
    useCreateGandengan();
  const { mutateAsync: updateGandengan, isLoading: isLoadingUpdate } =
    useUpdateGandengan();
  ...
  const { mutateAsync: deleteGandengan, isLoading: isLoadingDelete } =
    useDeleteGandengan();
  ...
  const [rows, setRows] = useState<IGandengan[]>([]);
  ...
  const streamBufferRef = useRef<Map<number, IGandengan[]>>(new Map());
  ...
  const forms = useForm<GandenganInput>({
    resolver: mode === 'delete' ? undefined : zodResolver(gandenganSchema),
    mode: 'onSubmit',
    defaultValues: {
      nama: '',
      keterangan: '',
  ...
  const { data: allGandengan, isLoading: isLoadingGandengan } = useGetGandengan(
    {
      ...filters,
  ...
  const columns = useMemo((): Column<IGandengan>[] => {
    return [
      {
        key: 'nomor',
        name: 'NO',
  ...
  function handleCellClick(args: { row: IGandengan }) {
  ...
  const handleGoToLastPage = useCallback(async () => {
    ...
    try {
      const results = await Promise.all(
        pagesToFetch.map((p) =>
          getGandenganFn({ ...filters, page: p, limit: filters.limit })
        )
      );

      const newCache = new Map<number, IGandengan[]>();
  ...
  const onSuccess = async (
    indexOnPage: number,
    fetchedPages: number[],
    pagedData: Record<string, IGandengan[]>,
    pageNumber: number,
    keepOpenModal = false,
    ...
    try {
      ...
      if (mode !== 'delete') {
        const response = await api2.get(
          `/redis/get/gandengan-page-${pageNumber}`
        );
        setRows([]);
        ...
        Object.entries(pagedData).forEach(([key, value]) => {
          updatedBuffer.set(Number(key), value as IGandengan[]);
        });
  ...
  const onSubmit = async (values: GandenganInput, keepOpenModal = false) => {
    clearError();
    ...
    try {
      ...
      if (mode === 'delete') {
        if (selectedRowId) {
          await deleteGandengan(selectedRowId as unknown as string, {
            onSuccess: () => {
              setPopOver(false);
      ...
      if (mode === 'add') {
        const newOrder = await createGandengan(
          {
            ...values,
            ...filters
          },
      ...
      if (selectedRowId && mode === 'edit') {
        ...
        queryClient.invalidateQueries('gandengans');
      }
    } catch (error: any) {
  ...
  const handleReport = async () => {
    try {
      ...
      const response = await getGandenganFn(filtersWithoutLimit);
      const reportRows = response.data.map((row) => ({
        ...row,
        judullaporan: 'Laporan Gandengan',
      ...
      import('stimulsoft-reports-js/Scripts/stimulsoft.blockly.editor')
        .then((module) => {
          ...
          report.loadFile('/reports/LaporanGandengan.mrt');
          ...
          report.renderAsync(() => {
            report.exportDocumentAsync((pdfData: any) => {
              ...
              window.open('/reports/gandengan', '_blank');
  ...
  function getRowClass(row: IGandengan) {
    const rowIndex = rows.findIndex((r) => r.id === row.id);
    return rowIndex === selectedRow ? 'selected-row' : '';
  }

  function rowKeyGetter(row: IGandengan) {
    return row.id;
  }
  ...
  const handleAdd = async () => {
    try {
      ...
      console.error('Error add gandengan:', error);
    }
  };
  ...
  const prefetchPages = useCallback(
    async (
      pagesToFetch: number[],
      existingCache?: Map<number, IGandengan[]>,
      ...
      await Promise.allSettled(
        validPages.map(async (pageNum) => {
          try {
            const data = await getGandenganFn({
              ...filters,
              page: pageNum,
              limit: filters.limit
            });
  ...
  useEffect(() => {
    const handleBulkFetch = async () => {
      ...
      const bulkData = allGandengan.data || [];
      ...
      const totalItems = allGandengan.pagination?.totalItems || 0;
      ...
      >
        ...
        <div className="flex flex-row justify-between border border-x-0 border-b-0 border-border bg-background-grid-header p-2">
          <ActionButton
            ...
            totalItems={allGandengan ? allGandengan.pagination.totalItems : 0}
            startRow={startRow}
            customActions={[
              {
                label: 'Print',
