...
import { IAllKaryawan, IKaryawan } from '../types/karyawan.type';
...
export const getKaryawanFn = async (
  filters: GetParams = {},
  signal?: AbortSignal
  ...
  try {
    ...
    const response = await api2.get('/karyawan', {
      params: queryParams,
      signal
    });
...
export const updateKaryawanFn = async ({ id, fields }: UpdateMenuParams) => {
  // controller karyawan memakai @Put(':id'), bukan @Put('update/:id') seperti jabatan
  const response = await api2.put(`/karyawan/${id}`, fields);
  return response.data;
};

export const storeKaryawanFn = async (fields: KaryawanInput) => {
  const response = await api2.post(`/karyawan`, fields);
