...
import { exportAsalKapalFn } from '@/lib/apis/asalkapal.api';
...
const ReportMenuPage: React.FC = () => {
  ...
  const handleExport = async () => {
    try {
      ...
      const response = await exportAsalKapalFn(exportPayload);

      // Buat link download dari Blob
      const url = window.URL.createObjectURL(new Blob([response]));
      const link = document.createElement('a');
      link.href = url;
      link.download = `laporan_asalkapal_${Date.now()}.xlsx`;
      ...
      console.error('Error exporting asalkapal data:', error);
    }
  };
