...
import { exportBiayaHeaderFn } from '@/lib/apis/biayaheader.api';
...
const ReportMenuPage: React.FC = () => {
  ...
  const handleExport = async () => {
    try {
      const response = await exportBiayaHeaderFn(Number(savedId));

      // Buat link download dari Blob
      const url = window.URL.createObjectURL(new Blob([response]));
      const link = document.createElement('a');
      link.href = url;
      link.download = `laporan_biaya_header_${Date.now()}.xlsx`;
