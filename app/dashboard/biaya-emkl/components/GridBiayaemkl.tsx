...
const GridBiayaEmkl = () => {
    ...
    return [
      ...
      },
      {
        ...
        renderHeaderCell: (column: any) => (
          <div className="flex h-full cursor-pointer flex-col items-center gap-1">
            ...
            >
              <p
                className={`text-sm ${
                  filters.sortBy === 'biaya_text' ? 'font-bold' : 'font-normal'
                }`}
              >
                BIAYA
              </p>
            ...
            <div className="relative h-[50%] w-full px-1">
              <FilterInput
                colKey="biaya_text"
                value={filters.filters.biaya_text || ''}
                onChange={(value) =>
                  handleFilterInputChange('biaya_text', value)
                }
                onClear={() => handleClearFilter('biaya_text')}
                inputRef={(el) => {
        ...
        renderCell: (props: any) => {
          ...
          const cellValue = props.row.biaya_text || '';
          return (
            <TooltipProvider delayDuration={0}>
              <Tooltip>
                <TooltipTrigger asChild>
      ...
      },
      {
        ...
        renderHeaderCell: (column: any) => (
          <div className="flex h-full cursor-pointer flex-col items-center gap-1">
            ...
            >
              <p
                className={`text-sm ${
                  filters.sortBy === 'coahut_text' ? 'font-bold' : 'font-normal'
                }`}
              >
                COA HUTANG
              </p>
              <div className="ml-2">
                {filters.sortBy === 'coahut_text' &&
                filters.sortDirection === 'asc' ? (
                  <FaSortUp className="font-bold" />
                ) : filters.sortBy === 'coahut_text' &&
                  filters.sortDirection === 'desc' ? (
            ...
            <div className="relative h-[50%] w-full px-1">
              <FilterInput
                ...
                inputRef={(el) => {
                  inputColRefs.current['coahut_text'] = el;
                }}
              />
            </div>
          </div>
        ...
        renderCell: (props: any) => {
          ...
          const cellValue = props.row.coahut_text || '';
          return (
            <TooltipProvider delayDuration={0}>
              <Tooltip>
                <TooltipTrigger asChild>
  ...
  async function handleScroll(event: React.UIEvent<HTMLDivElement>) {
    if (isLoadingBiaya || !hasMore || rows.length === 0) return;
  ...
  const onSuccess = async (
    ...
    try {
      if (keepOpenModal) {
        // Modal tetap terbuka untuk entri berikutnya, jadi form harus dikembalikan
        // ke nilai add yang lengkap — forms.reset() polos akan mengosongkan
        // statusbiayabl/statusseal/statustagih yang tidak ada di defaultValues.
        await resetAddForm();
        setPopOver(true);
      } else {
        forms.reset();
  ...
  const handleExport = async () => {
    try {
      ...
      link.href = url;
      link.download = `laporan_biaya${Date.now()}.xlsx`; // Nama file yang diunduh
  ...
  const lookupDefaultsRef = useRef<{
    aktif: LookupDefault;
    nilai: LookupDefault;
  } | null>(null);

  // STATUS BIAYA BL / SEAL / TAGIH ikut di-reset di sini. LookUp memang mengisinya
  // sendiri dari default redux, tapi reset() berjalan setelah dialog ter-mount dan
  // menghapusnya lagi — sementara LookUp tidak mengisi ulang karena inputValue
  // internalnya sudah terisi. Akibatnya field tampak "YA" padahal nilainya undefined.
  const fetchLookupDefaults = async () => {
    if (lookupDefaultsRef.current) return lookupDefaultsRef.current;

    const load = async (grp: string, fallbackText: string) => {
      const res = await api2.get('/parameter', { params: { grp } });
      const rows: any[] = res?.data?.data ?? res?.data ?? [];
      return pickLookupDefault(rows, fallbackText);
    };

    try {
      const [aktif, nilai] = await Promise.all([
        load('status aktif', 'AKTIF'),
        load('status nilai', 'YA')
      ]);
      lookupDefaultsRef.current = { aktif, nilai };
    } catch (e) {
      console.error('Gagal mengambil default lookup parameter:', e);
      return { aktif: EMPTY_LOOKUP_DEFAULT, nilai: EMPTY_LOOKUP_DEFAULT };
    }

    return lookupDefaultsRef.current;
  };

  const resetAddForm = async () => {
    const { aktif, nilai } = await fetchLookupDefaults();
    forms.reset(buildAddFormValues(aktif, nilai));
  };
  ...
  useEffect(() => {
    ...
    if (selectedRow !== null && rows.length > 0 && mode !== 'add') {
      ...
      forms.setValue('biaya_id', String(rowData?.biaya_id));
      ...
      forms.setValue('statusbiayabl', rowData?.statusbiayabl ?? '');
      forms.setValue('statusbiayabl_text', rowData?.statusbiayabl_text ?? '');

      forms.setValue('statusseal', rowData?.statusseal ?? '');
      forms.setValue('statusseal_text', rowData?.statusseal_text ?? '');

      forms.setValue('statustagih', rowData?.statustagih ?? '');
      ...
      <div className="flex h-[100%] w-full flex-col rounded-sm border border-border bg-background">
        ...
        <div className="flex flex-row justify-between border border-x-0 border-b-0 border-border bg-background-grid-header p-2">
          <ActionButton
            ...
            totalItems={allBiayaemkl ? allBiayaemkl.pagination.totalItems : 0}
            customActions={[
              {
                label: 'Print',
                icon: <FaPrint />,
            ...
            // ]}
          />
          {isLoadingBiaya ? <LoadRowsRenderer /> : null}
          {contextMenu && (
            <div
              ref={contextMenuRef}
