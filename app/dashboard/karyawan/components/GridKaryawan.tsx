...
import {
  KaryawanInput,
  karyawanSchema
} from '@/lib/validations/karyawan.validation';
...
import { IKaryawan } from '@/lib/types/karyawan.type';
...
import { getKaryawanFn } from '@/lib/apis/karyawan.api';
...
const GridKaryawan = () => {
  ...
  const { mutateAsync: createKaryawan, isLoading: isLoadingCreate } =
    useCreateKaryawan();
  const { mutateAsync: updateKaryawan, isLoading: isLoadingUpdate } =
    useUpdateKaryawan();
  const [currentPage, setCurrentPage] = useState(1);
  const [inputValue, setInputValue] = useState<string>('');
  const [hasMore, setHasMore] = useState(true);
  const inputRef = useRef<HTMLInputElement | null>(null);
  const { mutateAsync: deleteKaryawan, isLoading: isLoadingDelete } =
    useDeleteKaryawan();
  ...
  const [rows, setRows] = useState<IKaryawan[]>([]);
  ...
  const forms = useForm<KaryawanInput>({
  ...
  const { data: allKaryawan, isLoading: isLoadingKaryawan } = useGetKaryawan(
    {
      ...filters,
      page: currentPage
    },
  ...
  const columns = useMemo((): Column<IKaryawan>[] => {
    return [
      {
        key: 'nomor',
        name: 'NO',
      ...
      {
        // backend mengirim nama jabatan sebagai alias `jabatan_nama`
        // (leftJoin jabatan) dan filternya juga memakai key itu
        key: 'jabatan_nama',
        name: 'Jabatan',
        resizable: true,
        draggable: true,
        ...
        renderHeaderCell: (column: any) => (
          <div className="flex h-full cursor-pointer flex-col items-center gap-1">
            <div
              className="headers-cell h-[50%]"
              onClick={() => handleSort('jabatan_nama')}
              onContextMenu={(event) =>
                setContextMenu(handleContextMenu(event))
              }
            >
            ...
            <div className="relative h-[50%] w-full px-1">
              <FilterInput
                colKey="jabatan_nama"
                value={filters.filters.jabatan_nama || ''}
                onChange={(value) =>
                  handleFilterInputChange('jabatan_nama', value)
                }
                onClear={() => handleClearFilter('jabatan_nama')}
                inputRef={(el) => {
                  inputColRefs.current['jabatan_nama'] = el;
                }}
              />
            </div>
          </div>
        ...
        renderCell: (props: any) => {
          const columnFilter = filters.filters.jabatan_nama || '';
          const cellValue = props.row.jabatan_nama || '';
          return (
            <TooltipProvider delayDuration={0}>
              <Tooltip>
                <TooltipTrigger asChild>
      ...
      },
      {
        ...
        renderHeaderCell: (column: any) => (
          <div className="flex h-full cursor-pointer flex-col items-center gap-1">
            ...
            >
              <p
                className={`text-sm ${
                  filters.sortBy === 'kodeabsen' ? 'font-bold' : 'font-normal'
                }`}
              >
                Kode Absen
              </p>
              <div className="ml-2">
                {filters.sortBy === 'kodeabsen' &&
                filters.sortDirection === 'asc' ? (
                  <FaSortUp className="font-bold" />
                ) : filters.sortBy === 'kodeabsen' &&
                  filters.sortDirection === 'desc' ? (
            ...
            <div className="relative h-[50%] w-full px-1">
              <FilterInput
                colKey="kodeabsen"
                value={filters.filters.kodeabsen || ''}
                onChange={(value) =>
                  handleFilterInputChange('kodeabsen', value)
                }
                onClear={() => handleClearFilter('kodeabsen')}
                inputRef={(el) => {
                  inputColRefs.current['kodeabsen'] = el;
                }}
              />
            </div>
          </div>
        ...
        renderCell: (props: any) => {
          const columnFilter = filters.filters.kodeabsen || '';
          const cellValue = props.row.kodeabsen || '';
          return (
            <TooltipProvider delayDuration={0}>
              <Tooltip>
                <TooltipTrigger asChild>
  ...
  const onColumnResize = (index: number, width: number) => {
    ...
    resizeDebounceTimeout.current = setTimeout(() => {
      saveGridConfig(user.id, 'GridKaryawan', [...columnsOrder], newWidthMap);
    }, 300);
  };
  const onColumnsReorder = (sourceKey: string, targetKey: string) => {
    setColumnsOrder((prevOrder) => {
      ...
      saveGridConfig(user.id, 'GridKaryawan', [...newOrder], columnsWidth);
      return newOrder;
    });
  };
  ...
  async function handleScroll(event: React.UIEvent<HTMLDivElement>) {
    if (isLoadingKaryawan || !hasMore || rows.length === 0) return;
  ...
  function handleCellClick(args: { row: IKaryawan }) {
  ...
  async function handleKeyDown(
    args: CellKeyDownArgs<IKaryawan>,
  ...
  const onSuccess = async (
    ...
    try {
      ...
      if (mode !== 'delete') {
        const response = await api2.get(`/redis/get/karyawan-allItems`);
  ...
  const onSubmit = async (values: KaryawanInput, keepOpenModal = false) => {
    clearError();
    const selectedRowId = rows[selectedRow]?.id;
    try {
      ...
      if (mode === 'delete') {
        if (selectedRowId) {
          await deleteKaryawan(selectedRowId as unknown as string, {
            onSuccess: () => {
              setPopOver(false);
              setRows((prevRows) =>
            ...
            // backend membalas 400 dengan pesan jelas saat karyawan masih
            // dipakai tabel lain; tanpa ini kegagalan hapus tak terlihat sama
            // sekali karena catch di bawah membiarkan status 400 lewat diam-diam
            onError: (error: any) => {
      ...
      if (mode === 'add') {
        const newOrder = await createKaryawan(
          {
            ...values,
  ...
  const handleReport = async () => {
    try {
      ...
      const response = await getKaryawanFn(filtersWithoutLimit);
      const reportRows = response.data.map((row) => ({
        ...row,
        judullaporan: 'Laporan Karyawan',
      ...
      import('stimulsoft-reports-js/Scripts/stimulsoft.blockly.editor')
        .then((module) => {
          ...
          report.loadFile('/reports/LaporanKaryawan.mrt');
          ...
          report.renderAsync(() => {
            ...
            report.exportDocumentAsync((pdfData: any) => {
              ...
              window.open('/reports/laporankaryawan', '_blank');
  ...
  function getRowClass(row: IKaryawan) {
    const rowIndex = rows.findIndex((r) => r.id === row.id);
    return rowIndex === selectedRow ? 'selected-row' : '';
  }

  function rowKeyGetter(row: IKaryawan) {
    return row.id;
  }
  ...
  const resetAddForm = async () => {
    ...
    forms.reset({
      ...
      keterangan: '',
      kodeabsen: '',
      // karyawan_id dikosongkan: kolomnya punya FK self-reference ke
      // karyawan(id), jadi sentinel lama '0' selalu ditolak FK. Backend
      // menormalkan kosong/'0' menjadi NULL.
      karyawan_id: '',
      // absen_id tak punya FK; seluruh baris existing memakai '0'
      absen_id: '0',
  ...
  useEffect(() => {
    if (!allKaryawan || isDataUpdated) return;

    const newRows = allKaryawan.data || [];
    ...
    if (allKaryawan.pagination.totalPages) {
      setTotalPages(allKaryawan.pagination.totalPages);
    }

    setHasMore(newRows.length === filters.limit);
    setFetchedPages((prev) => new Set(prev).add(currentPage));
    setPrevFilters(filters);
  }, [allKaryawan, currentPage, filters, isFetchingManually, isDataUpdated]);
  ...
  useEffect(() => {
    ...
    if (
      ...
      // semua id di sini kolomnya text di PG & z.string() di skema — JANGAN
      // dibungkus Number(), itu bikin validasi gagal diam-diam sehingga submit
      // (termasuk delete) tak pernah jalan
      forms.setValue('jabatan_id', String(rowData?.jabatan_id ?? ''));
      forms.setValue('jabatan_nama', rowData?.jabatan_nama || '');
      forms.setValue('kodeabsen', rowData?.kodeabsen || '');
      forms.setValue('karyawan_id', String(rowData?.karyawan_id ?? ''));
      forms.setValue('absen_id', String(rowData?.absen_id ?? ''));
    }
      ...
      <div className="flex h-[100%] w-full flex-col rounded-sm border border-border bg-background">
        ...
        <div className="flex flex-row justify-between border border-x-0 border-b-0 border-border bg-background-grid-header p-2">
          <ActionButton
            ...
            totalItems={allKaryawan ? allKaryawan.pagination.totalItems : 0}
            customActions={[
              {
                label: 'Print',
                icon: <FaPrint />,
            ...
            // ]}
          />
          {isLoadingKaryawan ? <LoadRowsRenderer /> : null}
          {contextMenu && (
            <div
              ref={contextMenuRef}
