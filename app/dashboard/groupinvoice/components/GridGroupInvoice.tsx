...
import {
  filterGroupInvoice,
  GroupInvoice
} from '@/lib/types/groupinvoice.type';
import {
  useCreateGroupInvoice,
  useDeleteGroupInvoice,
  useGetAllGroupInvoice,
  useUpdateGroupInvoice
} from '@/lib/server/useGroupInvoice';
import FormGroupInvoice from './FormGroupInvoice';
import {
  GroupInvoiceInput,
  groupinvoiceSchema
} from '@/lib/validations/groupinvoice.validation';
import {
  checkValidationGroupInvoiceFn,
  getAllGroupInvoiceFn
} from '@/lib/apis/groupinvoice.api';
...
interface Filter {
  ...
  filters: typeof filterGroupInvoice;
}

const GridGroupInvoice = () => {
  ...
  const [rows, setRows] = useState<GroupInvoice[]>([]);
  ...
  const { data: allGroupInvoice, isLoading: isLoadingGroupInvoice } =
    useGetAllGroupInvoice({ ...filters, page: currentPage });
  const { mutateAsync: createGroupInvoice, isLoading: isLoadingCreate } =
    useCreateGroupInvoice();
  const { mutateAsync: updateGroupInvoice, isLoading: isLoadingUpdate } =
    useUpdateGroupInvoice();
  const { mutateAsync: deleteGroupInvoice, isLoading: isLoadingDelete } =
    useDeleteGroupInvoice();

  const forms = useForm<GroupInvoiceInput>({
    resolver: zodResolver(groupinvoiceSchema),
    mode: 'onSubmit',
    defaultValues: {
      kode: '',
      keterangan: '',
  ...
  const columns = useMemo((): Column<GroupInvoice>[] => {
    return [
      {
        key: 'nomor',
        name: 'NO',
      ...
      },
      {
        ...
        renderCell: ({ row }: { row: GroupInvoice }) => (
  ...
  const handleEdit = async () => {
    if (selectedRow !== null) {
      ...
      const result = await checkValidationGroupInvoiceFn({
        aksi: 'EDIT',
        value: rowData.id
      });
  ...
  const handleMultipleDelete = async (idsToDelete: number[]) => {
    try {
      for (const id of idsToDelete) {
        ...
        await deleteGroupInvoice(id as unknown as string);
      }
    ...
    try {
      ...
      if (checkedRows.size === 0) {
        ...
        const validationPromises = checkedRowsArray.map(async (id) => {
          try {
            const response = await checkValidationGroupInvoiceFn({
              aksi: 'DELETE',
              value: id
            });
            return {
  ...
  const onSuccess = async (
    ...
    try {
      if (keepOpenModal) {
        ...
        // setRows([]);
        if (mode !== 'delete') {
          const response = await api2.get(`/redis/get/groupinvoice-allItems`);
  ...
  const onSubmit = async (
    ...
    try {
      ...
      if (mode === 'delete') {
        if (selectedRowId) {
          await deleteGroupInvoice(selectedRowId as unknown as string, {
            onSuccess: () => {
              setPopOver(false);
              setRows((prevRows) =>
      ...
      if (mode === 'add') {
        const newOrder = await createGroupInvoice(
          {
            ...values,
      ...
      if (selectedRowId && mode === 'edit') {
        ...
        queryClient.invalidateQueries('groupinvoice');
      }
    } catch (error: any) {
  ...
  const handleReport = async () => {
    try {
      ...
      const response = await getAllGroupInvoiceFn(filtersWithoutLimit);
      const reportRows = response.data.map((row) => ({
        ...row,
        judullaporan: 'Laporan Group Invoice',
      ...
      import('stimulsoft-reports-js/Scripts/stimulsoft.blockly.editor')
        .then((module) => {
          ...
          report.loadFile('/reports/LaporanGroupInvoice.mrt');
          ...
          report.renderAsync(() => {
            ...
            report.exportDocumentAsync((pdfData: any) => {
              ...
              window.open('/reports/groupinvoice', '_blank');
  ...
  function handleCellClick(args: { row: GroupInvoice }) {
  ...
  async function handleScroll(event: React.UIEvent<HTMLDivElement>) {
    if (isLoadingGroupInvoice || !hasMore || rows.length === 0) return;
  ...
  async function handleKeyDown(
    args: CellKeyDownArgs<GroupInvoice>,
  ...
  function getRowClass(row: GroupInvoice) {
    const rowIndex = rows.findIndex((r) => r.id === row.id);
    return rowIndex === selectedRow ? 'selected-row' : '';
  }

  function rowKeyGetter(row: GroupInvoice) {
    return row.id;
  }
  ...
  useEffect(() => {
    if (!allGroupInvoice || isDataUpdated) return;

    const newRows = allGroupInvoice.data || [];
    ...
    if (allGroupInvoice.pagination.totalPages) {
      setTotalPages(allGroupInvoice.pagination.totalPages);
    }

    setHasMore(newRows.length === filters.limit);
    setFetchedPages((prev) => new Set(prev).add(currentPage));
    setPrevFilters(filters);
  }, [allGroupInvoice, currentPage, filters, isDataUpdated]);
  ...
  useEffect(() => {
    if (isSubmitSuccessful) {
      // reset();
      requestAnimationFrame(() => setFocus('kode')); // Pastikan fokus terjadi setelah repaint
    }
      ...
      <div className="flex h-[100%] w-full flex-col rounded-sm border border-border bg-background">
        ...
        <div className="flex flex-row justify-between border border-x-0 border-b-0 border-border bg-background-grid-header p-2">
          <ActionButton
            ...
            totalItems={
              allGroupInvoice ? allGroupInvoice.pagination.totalItems : 0
            }
            customActions={[
              {
                label: 'Print',
          ...
          />
          {isLoadingGroupInvoice ? <LoadRowsRenderer /> : null}
          {contextMenu && (
            <div
              ref={contextMenuRef}
...
export default GridGroupInvoice;
