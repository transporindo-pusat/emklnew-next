...
import {
  checkValidationMarketingGroupFn,
  getMarketingGroupFn
} from '@/lib/apis/marketinggroup.api';
import {
  Tooltip,
  TooltipContent,
  TooltipProvider,
...
const GridMarketingGroup = () => {
    ...
    try {
      ...
      if (checkedRows.size === 0) {
        if (selectedRow !== null) {
          ...
          if (selectedRowId) {
            const validationResponse = await checkValidationMarketingGroupFn({
              aksi: 'DELETE',
              value: selectedRowId
            });
        ...
        const validationPromises = checkedRowsArray.map(async (id) => {
          try {
            const response = await checkValidationMarketingGroupFn({
              aksi: 'DELETE',
              value: id
            });
            return {
  ...
  const handleMultipleDelete = async (idsToDelete: number[]) => {
    try {
      ...
      for (const id of idsToDelete) {
        await deleteMarketingGroup(id as unknown as string);
      }
  ...
  const handleReport = async () => {
    try {
      ...
      const response = await getMarketingGroupFn(filtersWithoutLimit);
      const reportRows = response.data.map((row) => ({
        ...row,
        judullaporan: 'Laporan Marketing Group',
      ...
      import('stimulsoft-reports-js/Scripts/stimulsoft.blockly.editor')
        .then((module) => {
          ...
          report.loadFile('/reports/LaporanMarketingGroup.mrt');
          ...
          report.renderAsync(() => {
            ...
            report.exportDocumentAsync((pdfData: any) => {
              ...
              window.open('/reports/marketinggroup', '_blank');
      ...
      <div className="flex h-[100%] w-full flex-col rounded-sm border border-border bg-background">
        ...
        <div className="flex flex-row justify-between border border-x-0 border-b-0 border-border bg-background-grid-header p-2">
          <ActionButton
            ...
            totalItems={
              allMarketingGroup ? allMarketingGroup.pagination.totalItems : 0
            }
            customActions={[
              {
                label: 'Print',
