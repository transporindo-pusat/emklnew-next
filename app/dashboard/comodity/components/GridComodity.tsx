...
import FormComodity from './FormComodity';
...
const GridComodity = () => {
  ...
  const [pageDataCache, setPageDataCache] = useState<Map<number, IComodity[]>>(
    new Map()
  );
  ...
  const streamBufferRef = useRef<Map<number, IComodity[]>>(new Map());
  ...
  const forms = useForm<ComodityInput>({
    resolver: mode === 'delete' ? undefined : zodResolver(ComoditySchema),
    mode: 'onSubmit',
    defaultValues: {
      keterangan: '',
      rate: '',
      ...
      },
      {
        ...
        renderCell: (props: any) => {
          ...
          return (
            <div
              title={cellValue}
              className="m-0 flex h-full w-full cursor-pointer items-center justify-end p-0 text-sm"
            >
              {highlightText(cellValue, filters.search, columnFilter)}
            </div>
          );
  ...
  async function handleScroll(event: React.UIEvent<HTMLDivElement>) {
    if (isLoadingComodity || rows.length === 0 || isTransitioning || isFetching)
      return;
  ...
  const handleGoToLastPage = useCallback(async () => {
    ...
    try {
      const results = await Promise.all(
        pagesToFetch.map((p) =>
          getComodityFn({ ...filters, page: p, limit: filters.limit })
        )
      );

      const newCache = new Map<number, IComodity[]>();
  ...
  const onSuccess = async (
    indexOnPage: number,
    fetchedPages: number[],
    pagedData: Record<string, IComodity[]>,
    pageNumber: number,
    keepOpenModal = false,
    ...
    try {
      ...
      if (mode !== 'delete') {
        const response = await api2.get(
          `/redis/get/comodity-page-${pageNumber}`
        );
        setRows([]);
        ...
        Object.entries(pagedData).forEach(([key, value]) => {
          updatedBuffer.set(Number(key), value as IComodity[]);
        });
  ...
  const onSubmit = async (values: ComodityInput, keepOpenModal = false) => {
    ...
    try {
      ...
      if (selectedRowId && mode === 'edit') {
        ...
        queryClient.invalidateQueries('comoditys');
      }
    } catch (error: any) {
  ...
  const handleAdd = async () => {
    try {
      ...
      console.error('Error add comodity:', error);
    }
  };
  ...
  const prefetchPages = useCallback(
    async (
      pagesToFetch: number[],
      existingCache?: Map<number, IComodity[]>,
      ...
      await Promise.allSettled(
        validPages.map(async (pageNum) => {
          try {
            const data = await getComodityFn({
              ...filters,
              page: pageNum,
              limit: filters.limit
            });
  ...
  useEffect(() => {
    const handleBulkFetch = async () => {
      ...
      const bulkData = allComodity.data || [];
      ...
      const totalItems = allComodity.pagination?.totalItems || 0;
      ...
      >
        ...
        <div className="flex flex-row justify-between border border-x-0 border-b-0 border-border bg-background-grid-header p-2">
          <ActionButton
            ...
            totalItems={allComodity ? allComodity.pagination.totalItems : 0}
            startRow={startRow}
            customActions={[
              {
                label: 'Print',
