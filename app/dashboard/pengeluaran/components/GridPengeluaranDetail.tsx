...
import { getPengeluaranDetailFn } from '@/lib/apis/pengeluaranheader.api';
import {
  formatCurrency,
  handleContextMenu,
  loadGridConfig,
...
const GridPengeluaranDetail = ({
  ...
  const [pageDataCache, setPageDataCache] = useState<
    Map<number, PengeluaranDetail[]>
  >(new Map());
  const streamBufferRef = useRef<Map<number, PengeluaranDetail[]>>(new Map());
  ...
  const { data: detail, isLoading } = useGetPengeluaranDetail(queryParams);
  ...
  const mapDetailRows = useCallback(
    (data: any[] | undefined | null): PengeluaranDetail[] =>
      (data ?? []).map((item: any) => ({
        id: item.id,
        ...
        pengeluaranheader_nobukti: item.pengeluaranheader_nobukti,
        penerimaanheader_nobukti: item.penerimaanheader_nobukti,
        info: item.info,
  ...
  const prefetchPages = useCallback(
    async (
      pagesToFetch: number[],
      existingCache?: Map<number, PengeluaranDetail[]>,
      ...
      await Promise.allSettled(
        validPages.map(async (pageNum) => {
          try {
            const data = await getPengeluaranDetailFn({
              ...queryParams,
              page: pageNum,
              limit: filters.limit
            });
  ...
  useEffect(() => {
    ...
    const newCache = new Map<number, PengeluaranDetail[]>();
  ...
  useEffect(() => {
    ...
    // Query untuk page 0 tidak pernah dijalankan (guard di useGetPengeluaranDetail),
  ...
  useEffect(() => {
    const combinedRows: PengeluaranDetail[] = [];
        ...
        <div className="flex h-[38px] w-full flex-row items-center justify-between rounded-t-sm border-b border-border bg-background-grid-header px-2">
          ...
          <div className="flex flex-row items-center">
            <DraggableColumn
              ...
              gridName="GridPengeluaranDetail"
