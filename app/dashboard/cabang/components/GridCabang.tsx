...
import { ICabang } from '@/lib/types/cabang.type';
import {
  clearOpenName,
  setClearLookup
...
import { getCabangFn } from '@/lib/apis/cabang.api';
...
const GridCabang = () => {
  ...
  const [pageDataCache, setPageDataCache] = useState<Map<number, ICabang[]>>(
    new Map()
  );

  const { mutateAsync: createCabang, isLoading: isLoadingCreate } =
    useCreateCabang();
  const { mutateAsync: updateCabang, isLoading: isLoadingUpdate } =
    useUpdateCabang();
  ...
  const [rows, setRows] = useState<ICabang[]>([]);
  ...
  const streamBufferRef = useRef<Map<number, ICabang[]>>(new Map());
  ...
  const forms = useForm<CabangInput>({
    resolver: mode === 'delete' ? undefined : zodResolver(cabangSchema),
    mode: 'onSubmit',
    defaultValues: {
      kodecabang: '',
      nama: '',
  ...
  const { data: allCabang, isLoading: isLoadingCabang } = useGetCabang(
    {
      ...filters,
  ...
  const columns = useMemo((): Column<ICabang>[] => {
    return [
      {
        key: 'nomor',
        name: 'NO',
      ...
      },
      {
          ...
          >
            ...
            <div className="relative h-[50%] w-full px-1">
              <FilterInput
                ...
                onChange={(value) =>
                  handleFilterInputChange('kodecabang', value)
                }
                onClear={() => handleClearFilter('kodecabang')}
                inputRef={(el) => {
                  inputColRefs.current['kodecabang'] = el;
                }}
              />
            </div>
          </div>
        ...
        renderCell: (props: any) => {
          ...
          const cellValue = props.row.kodecabang || '';
          return (
            <div
              title={cellValue}
  ...
  async function handleScroll(event: React.UIEvent<HTMLDivElement>) {
    if (isLoadingCabang || rows.length === 0 || isTransitioning || isFetching)
      return;
  ...
  function handleCellClick(args: { row: ICabang }) {
  ...
  const handleGoToLastPage = useCallback(async () => {
    ...
    try {
      const results = await Promise.all(
        pagesToFetch.map((p) =>
          getCabangFn({ ...filters, page: p, limit: filters.limit })
        )
      );

      const newCache = new Map<number, ICabang[]>();
  ...
  const onSuccess = async (
    indexOnPage: number,
    fetchedPages: number[],
    pagedData: Record<string, ICabang[]>,
    pageNumber: number,
    keepOpenModal = false,
    ...
    try {
      ...
      if (mode !== 'delete') {
        const response = await api2.get(`/redis/get/cabang-page-${pageNumber}`);
        setRows([]);
        ...
        Object.entries(pagedData).forEach(([key, value]) => {
          updatedBuffer.set(Number(key), value as ICabang[]);
        });
  ...
  const onSubmit = async (values: CabangInput, keepOpenModal = false) => {
    clearError();
    ...
    try {
      ...
      if (mode === 'add') {
        const newOrder = await createCabang(
          {
            ...values,
            ...filters
          },
  ...
  const handleReport = async () => {
    try {
      ...
      const response = await getCabangFn(filtersWithoutLimit);
      ...
      import('stimulsoft-reports-js/Scripts/stimulsoft.blockly.editor')
        .then((module) => {
          ...
          report.loadFile('/reports/LaporanJeniscabang.mrt');
  ...
  function getRowClass(row: ICabang) {
    const rowIndex = rows.findIndex((r) => r.id === row.id);
    return rowIndex === selectedRow ? 'selected-row' : '';
  }

  function rowKeyGetter(row: ICabang) {
    return row.id;
  }
  ...
  const handleAdd = async () => {
    try {
      ...
      console.error('Error add cabang:', error);
    }
  };
  ...
  const prefetchPages = useCallback(
    async (
      pagesToFetch: number[],
      existingCache?: Map<number, ICabang[]>,
  ...
  useEffect(() => {
    const handleBulkFetch = async () => {
      if (!shouldBulkFetch || !allCabang || isDataUpdated || isAfterMutation) {
        return;
      }

      const bulkData = allCabang.data || [];
      ...
      const totalItems = allCabang.pagination?.totalItems || 0;
  ...
  useEffect(() => {
    ...
    if (allCabang.pagination?.totalPages) {
      setTotalPages(allCabang.pagination.totalPages);
    }
      ...
      >
        ...
        <div className="flex flex-row justify-between border border-x-0 border-b-0 border-border bg-background-grid-header p-2">
          <ActionButton
            ...
            totalItems={allCabang ? allCabang.pagination.totalItems : 0}
            startRow={startRow}
            customActions={[
              {
                label: 'Print',
          ...
          />
          {isLoadingCabang ? <LoadRowsRenderer /> : null}
          {contextMenu && (
            <div
              ref={contextMenuRef}
