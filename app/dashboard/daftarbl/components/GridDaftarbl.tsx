...
import FormDaftarbl from './FormDaftarbl';
...
const GridDaftarbl = () => {
  ...
  const [pageDataCache, setPageDataCache] = useState<Map<number, IDaftarbl[]>>(
    new Map()
  );
  ...
  const streamBufferRef = useRef<Map<number, IDaftarbl[]>>(new Map());
  ...
  const forms = useForm<DaftarblInput>({
    resolver: mode === 'delete' ? undefined : zodResolver(daftarblSchema),
    mode: 'onSubmit',
    defaultValues: {
      nama: '',
      keterangan: '',
  ...
  async function handleScroll(event: React.UIEvent<HTMLDivElement>) {
    if (isLoadingDaftarbl || rows.length === 0 || isTransitioning || isFetching)
      return;
  ...
  const handleGoToLastPage = useCallback(async () => {
    ...
    try {
      const results = await Promise.all(
        pagesToFetch.map((p) =>
          getDaftarblFn({ ...filters, page: p, limit: filters.limit })
        )
      );

      const newCache = new Map<number, IDaftarbl[]>();
  ...
  const onSuccess = async (
    indexOnPage: number,
    fetchedPages: number[],
    pagedData: Record<string, IDaftarbl[]>,
    pageNumber: number,
    keepOpenModal = false,
    ...
    try {
      ...
      if (mode !== 'delete') {
        const response = await api2.get(
          `/redis/get/daftarbl-page-${pageNumber}`
        );
        setRows([]);
        ...
        Object.entries(pagedData).forEach(([key, value]) => {
          updatedBuffer.set(Number(key), value as IDaftarbl[]);
        });
  ...
  const onSubmit = async (values: DaftarblInput, keepOpenModal = false) => {
    ...
    try {
      ...
      if (selectedRowId && mode === 'edit') {
        ...
        queryClient.invalidateQueries('daftarbls');
      }
    } catch (error: any) {
    ...
    try {
      ...
      import('stimulsoft-reports-js/Scripts/stimulsoft.blockly.editor')
        .then((module) => {
          ...
          report.loadFile('/reports/LaporanJenisdaftarbl.mrt');
  ...
  const handleAdd = async () => {
    try {
      ...
      console.error('Error add daftar bl:', error);
    }
  };
  ...
  const prefetchPages = useCallback(
    async (
      pagesToFetch: number[],
      existingCache?: Map<number, IDaftarbl[]>,
      ...
      await Promise.allSettled(
        validPages.map(async (pageNum) => {
          try {
            const data = await getDaftarblFn({
              ...filters,
              page: pageNum,
              limit: filters.limit
            });
  ...
  useEffect(() => {
    const handleBulkFetch = async () => {
      ...
      const bulkData = allDaftarbl.data || [];
      ...
      const totalItems = allDaftarbl.pagination?.totalItems || 0;
      ...
      >
        ...
        <div className="flex flex-row justify-between border border-x-0 border-b-0 border-border bg-background-grid-header p-2">
          <ActionButton
            ...
            totalItems={allDaftarbl ? allDaftarbl.pagination.totalItems : 0}
            startRow={startRow}
            customActions={[
              {
                label: 'Print',
