...
import { useGetEstimasiBiayaDetailInvoice } from '@/lib/server/useEstimasiBiayaHeader';
import {
  EstimasiBiayaDetailInvoice,
  filterEstimasiBiayaDetailInvoice
...
interface Filter {
  ...
  limit: number;
  search: string;
  filters: typeof filterEstimasiBiayaDetailInvoice;
  sortBy: string;
  sortDirection: 'asc' | 'desc';
}

const GridEstimasiBiayaDetailInvoice = ({ activeTab }: GridProps) => {
  ...
  const [rows, setRows] = useState<EstimasiBiayaDetailInvoice[]>([]);
  ...
  const [filters, setFilters] = useState<Filter>({
    ...
    limit: 30,
    search: '',
    filters: filterEstimasiBiayaDetailInvoice,
    sortBy: 'nama',
    sortDirection: 'asc'
  });
  ...
  const {
    data: allDataEstimasiBiayaDetailInvoice,
    isLoading: isLoadingEstimasiBiayaDetailInvoice,
    refetch
  } = useGetEstimasiBiayaDetailInvoice(headerData?.id ?? 0, activeTab, '', {
    ...filters,
    page: 1
  });
  ...
  const columns = useMemo((): Column<EstimasiBiayaDetailInvoice>[] => {
    return [
      {
        key: 'nomor',
        name: 'NO',
  ...
  const onColumnResize = (index: number, width: number) => {
    ...
    resizeDebounceTimeout.current = setTimeout(() => {
      saveGridConfig(
        user.id,
        'GridEstimasiBiayaDetailInvoice',
        [...columnsOrder],
        newWidthMap
      );
    }, 300);
  ...
  function handleCellClick(args: { row: EstimasiBiayaDetailInvoice }) {
  ...
  async function handleKeyDown(
    args: CellKeyDownArgs<EstimasiBiayaDetailInvoice>,
  ...
  function getRowClass(row: EstimasiBiayaDetailInvoice) {
    const rowIndex = rows.findIndex((r) => r.id === row.id);
    return rowIndex === selectedRow ? 'selected-row' : '';
  }

  function rowKeyGetter(row: EstimasiBiayaDetailInvoice) {
    return row.id;
  }
  ...
  useEffect(() => {
    if (allDataEstimasiBiayaDetailInvoice) {
      const formattedRows = allDataEstimasiBiayaDetailInvoice?.data?.map(
        (item: any) => ({
          id: item.id,
          nobukti: item.nobukti,
          estimasibiaya_id: item.estimasibiaya_id,
          link_id: item.link_id, // Updated to match the field name
          link_nama: item.link_nama, // Updated to match the field name
          biayaemkl_id: item.biayaemkl_id, // Updated to match the field name
          biayaemkl_nama: item.biayaemkl_nama, // Updated to match the field name
  ...
  }, [allDataEstimasiBiayaDetailInvoice, headerData?.id]);
    ...
    <div className={`flex h-[100%] w-full justify-center`}>
      <div className="flex h-[100%] w-full flex-col rounded-sm border border-border bg-background">
        <div className="flex h-[38px] w-full flex-row items-center justify-between rounded-t-sm border-b border-border bg-background-grid-header px-2">
          ...
          <div className="flex flex-row items-center">
            <DraggableColumn
              ...
              userId={user.id}
              gridName="GridEstimasiBiayaDetailInvoice"
        ...
        <div className="flex flex-row justify-between border border-x-0 border-b-0 border-border bg-background-grid-header p-2">
          {isLoadingEstimasiBiayaDetailInvoice ? <LoadRowsRenderer /> : null}
...
export default GridEstimasiBiayaDetailInvoice;
